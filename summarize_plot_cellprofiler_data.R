# summarize_plot_cellprofiler_data.R
...
mean_intens <- objectData %>% 
  ...
  summarise(nCells = n(),
            ...
            MeanVim = mean(Intensity_MeanIntensity_Vimentin),
            MeanVim_SD = sd(Intensity_MeanIntensity_Vimentin))
...
p_meanVim <- ggplot(objectData,
                      aes(x=Metadata_Treatment,
                          y=Intensity_MeanIntensity_Vimentin)) + 
  geom_violin(trim=FALSE) +
  facet_wrap(~Metadata_Sample) +
  stat_summary(fun.data=mean_sdl,
               geom="pointrange", color="red")

outputPlotVim = paste(objectName, "Vimentin mean plot.pdf")
...
outputPooledVim = paste(objectName, "Vimentin mean pooled plot.pdf")
